...
= 2.4.0 =
* New global setting: **Always show pin labels** (Map Settings → Map Markers). Turn it on to keep every pin's name label visible at all times instead of only on hover or when selected. Off by default, so existing maps behave exactly as before.

= 2.3.3 =
* Fixed switching map views leaving the previous view's pins, lines, and labels stacked on top of the new one.
* Fixed the disorienting drag on mobile: the map image and the pins/lines now move together in one smooth motion while you drag (they were desyncing and lagging), then re-render razor-sharp the moment you let go.
* Zooming no longer feels "floaty" — the pins now stay perfectly locked to the map through the whole zoom instead of drifting for a moment and snapping into place.
...
* Markers, the hotel pin, and labels now render at a **consistent, readable size on every device** (they were coming out tiny on small screens) — still zoom-independent.
* Capped the auto cover-fit zoom on narrow screens so the map stays crisper instead of over-magnified.
